...
@attr.s
class KwsAgcParams:
    delta = attr.ib(type=float, default=16) # scale parameter
    theta = attr.ib(type=float, default=0.5) # voice gain level
    theta_s = attr.ib(type=float, default=0.1) # noise gain level
    sigma_s = attr.ib(type=float, default=0.003) # start variance for speech
    mu_s = attr.ib(type=float, default=0.0025) # start expectation for speech
    sigma_b = attr.ib(type=float, default=0.003) # start variance for noise
    mu_b = attr.ib(type=float, default=0.00) # start mu for noise
    win_size = attr.ib(type=int, default=100) # size of filter window
    k_mu = attr.ib(type=float, default=0.4) # forgetting coef for expectation
    k_sigma = attr.ib(type=float, default=0.33) # forgetting coef for expectation
...
class KwsAgc:
    ...
    def __call__(self, data:np.ndarray, rate: int = 16000) -> np.ndarray:
        ...
        if is_int16:
           # convert to float
           result = result / self.int16.max
        params: KwsAgcParams = self.params
        win_size: int = int(rate/1000*self.params.win_size)
        sigma_s, mu_s = params.sigma_s, params.mu_s
        sigma_b, mu_b = params.sigma_b, params.mu_b
        ...
        #for r in range(win_size, win_size + 2, win_size):
            ...
            mu_s_n = params.k_mu * l + (1 - params.k_mu) * mu_s
            mu_b_n = params.k_mu * l + (1 - params.k_mu) * mu_b
            
            sigma_s_n = params.k_sigma * (l - mu_s_n) * (l - mu_s_n) + \
                        (1 - params.k_sigma) * sigma_s * sigma_s
            sigma_b_n = params.k_sigma * (l - mu_b_n) * (l - mu_b_n) + \
                        (1 - params.k_sigma) * sigma_b * sigma_b
            z_b = (l - mu_b_n) / math.sqrt(sigma_b_n)
            z_s = (l - mu_s_n) / math.sqrt(sigma_s_n)
            # print(str(s) + " s")
            # print(str(e) + " e")
            # print(str(l) + " l")
            # print(str(mu_s_n) + " mu_s_n")
            # print(str(mu_b_n) + " mu_b_n")
            # print(str(sigma_s_n) + " sigma_s_n")
            # print(str(sigma_b_n) + " sigma_b_n")
            # print(str(z_b) + " z_b")
            # print(str(z_s) + " z_s")
            ...
            if speech:
                ...
                if sigma_s_n < tau * tau:
                    sigma_s_n = sigma_s_n + (sigma_s_n + sigma_b_n) / (2 * params.delta)
                sigma_s = math.sqrt(sigma_s_n)
                gain = params.theta / (mu_s + sigma_s) if (mu_s - mu_b) > \
                ...
                if sigma_b_n < tau * tau:
                    sigma_b_n = sigma_b_n + (sigma_b_n + sigma_s_n) / (2 * self.params.delta)
